        ...
        <div className="space-y-6 px-5 py-5 sm:px-6">
          ...
          {schedule && (
            <>
              ...
              {pending.length === 0 && students.length === 0 && rejected.length === 0 && (
                <p className="rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-600">No appointment requests for this schedule yet.</p>
              )}

              <section>
                <h3 className="text-lg font-bold text-maroon-900">Pending Student Requests ({pending.length})</h3>
                {pending.length === 0 ? (
                  <p className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-600">No pending student requests for this schedule yet.</p>
                ) : (
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    {pending.map((appointment) => (
                      <article key={appointment._id} className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="flex min-w-0 items-center gap-3"><ProfileImagePreview user={appointment.student} className="h-11 w-11 rounded-full bg-maroon-800 font-bold text-white" buttonClassName="rounded-full" /><h4 className="break-words font-bold text-slate-900">{appointment.student?.name || "Student"}</h4></div>
                          <StatusBadge status={appointment.status} />
                        </div>
                        <p className="mt-3 text-sm text-slate-600">Estimated Time: {appointment.estimatedDurationMinutes || 0} min</p>
                        <p className="mt-1 break-words text-sm text-slate-600">Topic: {appointment.subject}</p>
                        <div className="mt-4 flex flex-wrap gap-2">
                          <button type="button" onClick={() => setAppointmentDetails(appointment)} className="btn-secondary">View Details</button>
                          <button type="button" disabled={submitting} onClick={async () => { setSubmitting(true); await onApprove(appointment); setSubmitting(false); }} className="rounded-lg bg-green-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-60">Approve</button>
                          <button type="button" disabled={submitting} onClick={async () => { const note = window.prompt("Optional rejection reason:") || ""; setSubmitting(true); await onReject(appointment, note); setSubmitting(false); }} className="rounded-lg bg-red-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-60">Reject</button>
                          <button type="button" disabled={submitting} onClick={() => setRescheduleTarget(appointment)} className="rounded-lg border border-amber-300 px-4 py-2 text-sm font-bold text-amber-800">Reschedule</button>
                        </div>
                      </article>
                    ))}
                  </div>
              ...
              <section>
                <h3 className="text-lg font-bold text-maroon-900">Rejected ({rejected.length})</h3>
                {rejected.length === 0 ? (
                  <p className="mt-4 text-sm text-slate-500">No rejected requests for this schedule.</p>
                ) : (
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                      ...
                      <article key={appointment._id} className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="flex min-w-0 items-center gap-3">
                            <ProfileImagePreview user={appointment.student} className="h-11 w-11 rounded-full bg-maroon-800 font-bold text-white" buttonClassName="rounded-full" />
                            <div className="min-w-0"><h4 className="break-words font-bold text-slate-900">{appointment.student?.name || "Student"}</h4><p className="mt-1 break-words text-sm text-slate-500">{appointment.subject}</p></div>
                          </div>
                          <StatusBadge status={appointment.status} />
                        </div>
                        <button type="button" onClick={() => setAppointmentDetails(appointment)} className="btn-secondary mt-4">View Details</button>
                      </article>
                    ))}
                  </div>
                )}
